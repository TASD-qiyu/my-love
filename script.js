...
function getUsers() {
  try { return JSON.parse(localStorage.getItem('love_users') || '{}'); }
  catch { return {}; }
}
function saveUsers(users) {
  localStorage.setItem('love_users', JSON.stringify(users));
}
...
function saveUserData(data) {
  if (!state.currentUser) return;
  const users = getUsers();
  users[state.currentUser] = { ...users[state.currentUser], ...data };
  saveUsers(users);
}
function loadUserData() {
  if (!state.currentUser) return;
  const data = getUserData();
  if (data.pattern) state.pattern = data.pattern;
  if (data.avatarLeft) { state.avatarLeft = data.avatarLeft; updateAvatar('avatarLeft', data.avatarLeft); }
  if (data.avatarRight) { state.avatarRight = data.avatarRight; updateAvatar('avatarRight', data.avatarRight); }
  if (data.lockBg) { state.lockBg = data.lockBg; $('lockBgLayer').style.backgroundImage = `url(${data.lockBg})`; }
  if (data.mainBg) { state.mainBg = data.mainBg; $('mainPage').style.backgroundImage = `url(${data.mainBg})`; }
  if (data.coupleBg) { state.coupleBg = data.coupleBg; $('coupleCardBg').style.backgroundImage = `url(${data.coupleBg})`; $('coupleHint').style.display = 'none'; }
  if (data.playerCover) { state.playerCover = data.playerCover; $('playerCover').style.backgroundImage = `url(${data.playerCover})`; $('playerCover').innerHTML = ''; }
  if (data.lockText) { state.lockText = data.lockText; $('lockTextDisplay').textContent = data.lockText; }
  if (data.lockTextColor) { state.lockTextColor = data.lockTextColor; $('lockTextDisplay').style.color = data.lockTextColor; }
  if (data.nick1) { state.nick1 = data.nick1; $('nick1').textContent = data.nick1; }
  if (data.nick2) { state.nick2 = data.nick2; $('nick2').textContent = data.nick2; }
  if (data.songName) { state.songName = data.songName; $('songNameDisplay').textContent = data.songName; }
  if (data.theme) { state.theme = data.theme; applyTheme(data.theme); }
  if (data.wordBank) state.wordBank = data.wordBank;
  if (data.savedMessages) state.savedMessages = data.savedMessages;
  if (data.memoList) state.memoList = data.memoList;
  if (data.diaryList) state.diaryList = data.diaryList;
  if (data.photoList) state.photoList = data.photoList;
  if (data.chatMessages) state.chatMessages = data.chatMessages;
  if (data.promiseVerified) state.promiseVerified = data.promiseVerified;
  if (data.anniversaryDate) state.anniversaryDate = data.anniversaryDate;
  if (data.currentSongIndex !== undefined) state.currentSongIndex = data.currentSongIndex;
}
function saveAll() {
  saveUserData({
    pattern: state.pattern,
    avatarLeft: state.avatarLeft,
    avatarRight: state.avatarRight,
    lockBg: state.lockBg,
    mainBg: state.mainBg,
    coupleBg: state.coupleBg,
    playerCover: state.playerCover,
    lockText: state.lockText,
    lockTextColor: state.lockTextColor,
    nick1: state.nick1,
    nick2: state.nick2,
    ...
    promiseVerified: state.promiseVerified,
...
// ========== 登录系统 ==========
function initLogin() {
  const lastUser = localStorage.getItem('love_last_user');
  if (lastUser) {
    state.currentUser = lastUser;
    $('loginOverlay').style.display = 'none';
    loadUserData();
    if (!state.promiseVerified) {
      $('promiseOverlay').style.display = 'flex';
    } else {
      initLockScreen();
    }
  }
...
$('loginBtn').onclick = function() {
  ...
  if (!user || !pass) { toast('请填写完整信息'); return; }
  const users = getUsers();
  if (state.isLoginMode) {
    if (!users[user]) { toast('用户不存在'); return; }
    if (users[user].password !== pass) { toast('密码错误'); return; }
    state.currentUser = user;
    localStorage.setItem('love_last_user', user);
    $('loginOverlay').style.display = 'none';
    loadUserData();
    if (!state.promiseVerified) {
      $('promiseOverlay').style.display = 'flex';
    ...
    if (users[user]) { toast('用户名已存在'); return; }
...
$('switchMode').onclick = function() {
  ...
  $('loginBtn').textContent = state.isLoginMode ? '登录' : '注册';
  $('switchMode').innerHTML = state.isLoginMode ? '还没有账号？<span>立即注册</span>' : '已有账号？<span>立即登录</span>';
};

// ========== 承诺验证 ==========
const PROMISE_TEXT = '我绝不盈利、造谣、污蔑或嘲讽，并对自己的使用行为负完全责任';
$('promiseBtn').onclick = function() {
  const input = $('promiseInput').value.trim();
  if (input === PROMISE_TEXT) {
    state.promiseVerified = true;
    saveUserData({ promiseVerified: true });
    $('promiseOverlay').style.display = 'none';
    initLockScreen();
    toast('欢迎进入');
  } else {
    $('promiseInput').classList.add('error');
    setTimeout(() => $('promiseInput').classList.remove('error'), 500);
    toast('请逐字抄写承诺');
  }
};
...
function initDots() {
  ...
  dots.forEach(dot => {
    ...
    dot.addEventListener('dblclick', () => showModal('patternModal'));
  });
}
function handleDotClick(dot) {
  ...
  if (state.unlockSequence.length >= state.pattern.length) {
    if (JSON.stringify(state.unlockSequence) === JSON.stringify(state.pattern)) {
      ...
      setTimeout(() => { dots.forEach(d => d.classList.remove('error')); state.unlockSequence = []; }, 400);
    }
  }
}
$('enterMainBtn').onclick = function() {
...
// ========== 图案设置 ==========
$('patternSave').onclick = function() {
  if (state.tempPattern.length >= 3) {
    state.pattern = [...state.tempPattern];
    saveUserData({ pattern: state.pattern });
    toast('密码已保存');
...
$('patternCancel').onclick = function() {
  state.tempPattern = [];
  document.querySelectorAll('.pattern-dot').forEach(d => d.classList.remove('selected'));
  hideModal('patternModal');
};
...
$('textEditSave').onclick = function() {
  ...
  if (text) {
    ...
    saveUserData({ lockText: text, lockTextColor: color });
    toast('已保存');
  }
...
$('lockBgUpload').onchange = function() {
  handleFileUpload(this, url => { state.lockBg = url; $('lockBgLayer').style.backgroundImage = `url(${url})`; });
};
$('mainBgUpload').onchange = function() {
  handleFileUpload(this, url => { state.mainBg = url; $('mainPage').style.backgroundImage = `url(${url})`; });
};
$('coupleBgUpload').onchange = function() {
  handleFileUpload(this, url => { state.coupleBg = url; $('coupleCardBg').style.backgroundImage = `url(${url})`; $('coupleHint').style.display = 'none'; });
};
$('playerCoverUpload').onchange = function() {
  handleFileUpload(this, url => { state.playerCover = url; $('playerCover').style.backgroundImage = `url(${url})`; $('playerCover').innerHTML = ''; });
};
$('lockBgSave').onclick = function() { saveUserData({ lockBg: state.lockBg }); toast('锁屏背景已保存'); hideModal('lockBgModal'); };
$('mainBgSave').onclick = function() { saveUserData({ mainBg: state.mainBg }); toast('主界面背景已保存'); hideModal('mainBgModal'); };
$('coupleBgSave').onclick = function() { saveUserData({ coupleBg: state.coupleBg }); toast('卡片背景已保存'); hideModal('coupleBgModal'); };
$('playerCoverSave').onclick = function() { saveUserData({ playerCover: state.playerCover }); toast('封面已保存'); hideModal('playerCoverModal'); };

// ========== 头像上传 ==========
function uploadAvatar(side) {
  const input = $('avatarUpload');
  input.onchange = function() {
    handleFileUpload(this, url => {
      state[side] = url;
      updateAvatar(side, url);
      saveUserData({ [side]: url });
    });
  };
  input.click();
}
...
$('nickSave').onclick = function() {
  ...
  if (val) {
    ...
    saveUserData({ nick1: state.nick1, nick2: state.nick2 });
    toast('昵称已保存');
  }
...
$('songNameSave').onclick = function() {
  const val = $('songNameInput').value.trim();
  if (val) { state.songName = val; $('songNameDisplay').textContent = val; saveUserData({ songName: val }); toast('歌曲名已保存'); }
...
function prevSong() {
  ...
  saveUserData({ currentSongIndex: state.currentSongIndex });
}
function nextSong() {
...
function startAnniversaryTimer() {
  if (!state.anniversaryDate) {
    ...
    saveUserData({ anniversaryDate: state.anniversaryDate });
  }
  updateAnniversary();
...
$('themeSaveBtn').onclick = function() {
  ...
  saveUserData({ theme: state.theme });
  toast('主题已保存');
...
function renderWordBank() {
  const bank = $('wordBank');
  bank.innerHTML = '';
  state.wordBank.forEach((word, i) => {
    const chip = document.createElement('div');
    chip.className = 'word-chip';
    chip.textContent = word;
    chip.onclick = () => addWordToDisplay(word, i);
...
function renderWordDisplay() {
  ...
  display.innerHTML = '';
  state.wordDisplay.forEach((word, i) => {
    const chip = document.createElement('div');
    chip.className = 'word-chip in-display';
    chip.textContent = word;
    chip.onclick = () => { state.wordDisplay.splice(i, 1); renderWordDisplay(); renderWordBank(); };
...
function addCustomWord() {
  ...
  if (val) {
    ...
    $('wordInput').value = '';
    renderWordBank();
    saveUserData({ wordBank: state.wordBank });
  }
}
function shuffleWords() {
  for (let i = state.wordBank.length - 1; i > 0; i--) {
    const j = Math.floor(Math.random() * (i + 1));
    [state.wordBank[i], state.wordBank[j]] = [state.wordBank[j], state.wordBank[i]];
  }
  renderWordBank();
}
...
function saveWordMessage() {
  ...
  saveUserData({ savedMessages: state.savedMessages });
...
function backupAll() {
  ...
  a.href = url;
  a.download = `love_backup_${state.currentUser}_${Date.now()}.json`;
  a.click();
...
function renderChat(container) {
  ...
  setTimeout(() => {
    state.chatMessages.forEach(m => appendChatBubble(m.text, m.isMe));
  }, 0);
}
function sendChat() {
  ...
  state.chatMessages.push({ text, isMe: true, time: Date.now() });
  saveUserData({ chatMessages: state.chatMessages });
  input.value = '';
  // 模拟回复
  ...
  input.onchange = function() {
    Array.from(this.files).forEach(file => {
      ...
      reader.onload = e => {
        ...
        saveUserData({ photoList: state.photoList });
        openApp('album');
      };
      reader.readAsDataURL(file);
    });
...
function renderCompanion(container) {
  container.innerHTML = '<div style="text-align:center;padding:40px 0;"><div style="font-size:60px;margin-bottom:16px;">💕</div><div style="font-size:18px;color:var(--accent);margin-bottom:8px;">已相伴</div><div style="font-size:36px;font-weight:700;">' + (state.anniversaryDate ? Math.floor((Date.now() - new Date(state.anniversaryDate)) / 86400000) : 0) + ' 天</div><div style="margin-top:20px;font-size:14px;color:var(--text-dim);">每一天都是特别的礼物</div></div>';
}

function renderMusicApp(container) {
  let html = '<div style="text-align:center;padding:20px 0;">';
  state.songs.forEach((song, i) => {
    html += `<div style="padding:12px;border-radius:12px;margin-bottom:8px;background:${i===state.currentSongIndex?'var(--accent)':'var(--glass)'};cursor:pointer;" onclick="playSong(${i})">${i===state.currentSongIndex?'▶ ':'　'}${song}</div>`;
  });
  html += '</div>';
  container.innerHTML = html;
}
function playSong(idx) {
  ...
  saveUserData({ currentSongIndex: idx });
  openApp('music');
}
...
function renderLetter(container) {
  ...
  container.innerHTML = `<div class="letter-container"><div class="letter-envelope" onclick="this.style.transform='translateY(-20px)';setTimeout(()=>{$('letterText').style.opacity=1},300)"></div><div class="letter-text" id="letterText" style="opacity:0;transition:opacity 0.5s;">${letter.replace(/\n/g,'<br>')}</div></div>`;
}
...
function saveDiary() {
  ...
  state.diaryList.unshift({ text, date: new Date().toLocaleString() });
  saveUserData({ diaryList: state.diaryList });
  toast('日记已保存');
  openApp('diary');
}
...
function renderCheckin(container) {
  ...
  container.innerHTML = `<div style="text-align:center;padding:40px 0;"><div style="font-size:60px;margin-bottom:16px;">${checked?'✅':'📍'}</div><div style="font-size:18px;margin-bottom:8px;">${checked?'今日已打卡':'今日未打卡'}</div><button class="btn-primary" style="width:200px;" ${checked?'disabled':''} onclick="doCheckin()">${checked?'已完成':'打卡'}</button><div style="margin-top:20px;font-size:14px;color:var(--text-dim);">连续打卡：<span style="color:var(--accent);font-weight:700;">${state.checkinStreak || 0}</span> 天</div></div>`;
}
function doCheckin() {
  const today = new Date().toDateString();
  ...
  saveUserData({ lastCheckin: today, checkinStreak: state.checkinStreak });
  toast('打卡成功！');
  openApp('checkin');
}
...
function addWish() {
  ...
  saveUserData({ wishList: state.wishList });
  openApp('gift');
}
...
function renderMemo(container) {
  ...
  setTimeout(() => {
    ...
    state.memoList.forEach((m, i) => {
      const item = document.createElement('div');
      item.className = 'memo-item';
      item.innerHTML = `<input type="checkbox" ${m.done?'checked':''} onchange="toggleMemo(${i})"><span class="${m.done?'done':''}">${m.text}</span><span class="memo-del" onclick="deleteMemo(${i})">×</span>`;
      list.appendChild(item);
    });
  }, 0);
}
function addMemo() {
  ...
  saveUserData({ memoList: state.memoList });
  openApp('memo');
}
...
function renderCalendar(container) {
  ...
  let html = '<div class="calendar-nav"><button onclick="">‹</button><span>' + year + '年' + (month + 1) + '月</span><button>›</button></div>';
  html += '<div class="calendar-grid">';
  '日一二三四五六'.split('').forEach(d => html += '<div class="day-header">' + d + '</div>');
...
function renderFood(container) {
  ...
  foods.forEach(f => html += `<div class="food-item" onclick="selectFood(this)">${f}</div>`);
...
function spinDecision() {
  ...
  const options = input ? input.split(/[,，]/).map(s => s.trim()).filter(s => s) : ['是','否','再想想'];
...
function renderGiftbox(container) {
  ...
  gifts.forEach((g, i) => html += `<div class="gift-item" onclick="openGift(this)"><div>${g.icon}</div><span>${g.name}</span></div>`);
...
function logout() {
  ...
  localStorage.removeItem('love_last_user');
